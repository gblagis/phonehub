...
@section('content')
  ...
  @if (filled(request('q')))
    <form class="grid grid-cols-2 md:grid-cols-7 gap-2 mb-4" method="GET" action="{{ route('listings.index') }}">
      ...
      <input name="min_price" value="{{ request('min_price') }}" type="number" class="border rounded p-2" placeholder="Min €"/>
      <input name="max_price" value="{{ request('max_price') }}" type="number" class="border rounded p-2" placeholder="Max €"/>
